...
# Set up logging and create results directories with a timestamp
...
def augment_audio(file_path: str, label: int, augmentation_count: int = 2) -> Tuple[List, List]:
    ...
    Parameters:
        file_path (str): Path to the audio file.
        label (int): Label for the audio.
        augmentation_count (int): Number of augmentations per method.
    
    Returns:
        tuple: (List of feature vectors, List of labels)
    """
    try:
        y, sr = librosa.load(file_path, sr=16000)
        if y.size == 0:
...
def prepare_dataset(use_augmentation: bool = True, class_balance: bool = True, use_test_mode: bool = False) -> tuple[np.ndarray, np.ndarray]:
    ...
    Parameters:
        ...
        class_balance (bool): Whether to apply SMOTE for balancing.
        use_test_mode (bool): If True, limits the number of files processed.
...
def feature_selection_analysis(X: np.ndarray, y: np.ndarray, use_test_mode: bool = False) -> dict:
    ...
    Parameters:
        ...
        y (np.ndarray): Labels.
        use_test_mode (bool): If True, uses simplified parameters.
...
def deep_rl_tune_svm(X_train, y_train, episodes=20):
    """
    Use a deep RL (REINFORCE) approach to tune SVM hyperparameters.
    
    Returns:
        tuple: Best (C, gamma) found.
    """
    ...
    for episode in range(episodes):
        ...
        print(f"Episode {episode+1}: C={C:.4f}, gamma={gamma:.4f}, reward={reward:.4f}")
        if reward > best_reward:
            best_reward = reward
            best_hyperparams = (C, gamma)
            
    print("Best hyperparameters found:", best_hyperparams, "with reward", best_reward)
...
def deep_rl_tune_ensemble_weights(X_val_scaled, y_val, models, episodes=20):
    """
    Use deep RL to tune ensemble weights for three models.
    
    Parameters:
        models (dict): Dictionary with keys 'SVM_TwoTier', 'SVM_1v1v1', and 'ET_1v1v1'.
    
    Returns:
        tuple: Best weights (w1, w2, w3) found.
    """
    ...
    for episode in range(episodes):
        ...
        print(f"Ensemble Episode {episode+1}: w1={w1:.4f}, w2={w2:.4f}, w3={w3:.4f}, reward={reward:.4f}")
        if reward > best_reward:
            best_reward = reward
            best_weights = (w1, w2, w3)
            
    print("Best ensemble weights found:", best_weights, "with reward", best_reward)
    return best_weights
...
def train_recommended_models_deeprl(X: np.ndarray, y: np.ndarray, feature_selection_results: dict, use_test_mode: bool = False) -> dict:
    ...
    # Tune tier1 SVM hyperparameters
    ...
    # For tier2, train only on chicken samples
    chicken_indices = np.where(y_tier1 == 0)[0]
    ...
    # Extra Trees with default deep RL–free tuning (or you can similarly add a deep RL tuner)
...
def train_enhanced_ensemble_deeprl(X: np.ndarray, y: np.ndarray, recommended_models: dict, use_test_mode: bool = False) -> dict:
    ...
    Returns:
        dict: Dictionary with ensemble model details and evaluation metrics.
    """
...
def final_model_evaluation(recommended_models: dict, ensemble_result: dict, X: np.ndarray, y: np.ndarray):
    ...
    Returns:
        The best performing model name.
    """
...
def main():
    ...
    try:
        logger.info("Starting Phase 5 experiment")
        overall_steps = 5
        ...
        logger.info("Step 3: Training recommended models (Deep RL tuning)")
