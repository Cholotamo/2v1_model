...
# -------------------------------
# Set up a device variable to use the GPU if available
...
class HyperparamController(nn.Module):
    def __init__(self):
        ...
        self.mu = nn.Parameter(torch.tensor([0.0, 0.0], device=device))
        self.log_std = nn.Parameter(torch.tensor([0.0, 0.0], device=device))
    
    def forward(self):
        std = torch.exp(self.log_std)
        eps = torch.randn(2, device=self.mu.device)  # Ensure tensor is created on the same device
...
def deep_rl_tune_svm(X_train, y_train, episodes=50):
    ...
    controller = HyperparamController().to(device)
...
class EnsembleWeightController(nn.Module):
    def __init__(self):
        ...
        self.mu = nn.Parameter(torch.tensor([0.3, 0.3], device=device))
        self.log_std = nn.Parameter(torch.tensor([0.0, 0.0], device=device))
    
    def forward(self):
        std = torch.exp(self.log_std)
        eps = torch.randn(2, device=self.mu.device)
        sample = self.mu + std * eps
...
def deep_rl_tune_ensemble_weights(X_val_scaled, y_val, models, episodes=50):
    ...
    controller = EnsembleWeightController().to(device)
    optimizer = optim.Adam(controller.parameters(), lr=0.01)
    best_reward = -np.inf
    best_weights = None
    reward_history = []
